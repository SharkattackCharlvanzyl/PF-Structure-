...
import { SALE_PROPERTIES } from "@/lib/properties";
...
export default function BuyDetailPage() {
  ...
  const property = SALE_PROPERTIES.find((p) => p.id === id);
  ...
  if (!property) {
    return (
      <div className="max-w-4xl mx-auto px-4 pt-24 pb-16 text-center">
        ...
        <Link href="/buy" className="text-gold hover:text-gold-light">{t("backToListings")}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pt-24 pb-16">
      ...
      <div className="flex items-center gap-2 text-sm text-cream/40 mb-6">
        <Link href="/buy" className="hover:text-gold transition-colors">{t("buy")}</Link>
        <span>/</span>
